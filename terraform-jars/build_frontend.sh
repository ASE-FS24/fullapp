...
export CHAT_API_ENDPOINT=$(terraform output -raw chatmanager_endpoint_url)
...
echo "Chat Manager API Endpoint: ${CHAT_API_ENDPOINT}"
...
sed -i '' "s|REACT_APP_CHAT_BASEURL=.*|REACT_APP_CHAT_BASEURL=${CHAT_API_ENDPOINT}|g" ../frontend/.env
...
if [ ! -z "${USER_API_ENDPOINT}" ] && [ ! -z "${POST_API_ENDPOINT}" ] && [ ! -z "${CHAT_API_ENDPOINT}" ] && [ ! -z "$(terraform output -raw frontend_url)" ] && [ ! -z "$(terraform output -raw cloudfront_url)" ]; then
